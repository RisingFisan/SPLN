...
ds = pd.read_csv("NomesMasculinos.csv").merge(pd.read_csv("NomesFemininos.csv"),how='outer')
ds.sort_values(by=['TOTAL'], inplace=True, ascending=False)
# print(ds.head())

nomes.update(set(ds['NOME']))

import pickle

with open("nomes.pck","wb") as f:
    pickle.dump(nomes, f)
